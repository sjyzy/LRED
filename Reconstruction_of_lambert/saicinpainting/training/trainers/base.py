...
class BaseInpaintingTrainingModule(ptl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        """Manual optimization: update generator and discriminator in one step."""
        ...
        out_g = self._do_step(batch, batch_idx, mode='train')
        loss_g = out_g['loss']
        ...
        if self.config.losses.adversarial.weight > 0:
            out_d = self._do_step(batch, batch_idx, mode='train', disc_step=True)
            loss_d = out_d['loss']
        ...
        # ---- Logging ----
        if 'log_info' in out_g:
            self.log_dict(out_g['log_info'], on_step=True, on_epoch=False, prog_bar=False)
        if out_d and 'log_info' in out_d:
            self.log_dict(out_d['log_info'], on_step=True, on_epoch=False, prog_bar=False)

        # PL 2.x 允许 training_step 返回 None
        return
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx):
        ...
        out = self._do_step(batch, batch_idx, mode=mode, extra_val_key=extra_val_key)

        # --- NEW: 缓存一份本 step 的输出，避免显存泄漏先搬到 CPU ---
        safe = {}
        if out is not None:
            ...
            # 记录来源（可选）
            safe['_mode'] = mode
            safe['_dataloader_idx'] = dataloader_idx
            self._val_epoch_outputs.append(safe)
        # -------------------------------------------------------------
    ...
    # 删掉 / 改名原来的 validation_epoch_end(self, outputs)
    def on_validation_epoch_end(self):
        ...
        # —— 以下基本保留你原本的实现 ——
        # 以前 outputs 是嵌套 list（多卡 × steps）；我们现在已经每步 append 一次，
        # 这里直接视作扁平列表用就行。如果你想保持完全一致，也可按原来再扁平一次：
        # outputs = [step_out for step_out in outputs]  # 已扁平，可省略
        ...
        if len(val_evaluator_states) > 0:
            ...
            LOGGER.info("Validation: no states collected in sanity/epoch; skip evaluator.")
        ...
        if len(test_evaluator_states) > 0:
            ...
            LOGGER.info("Test: no states collected in sanity/epoch; skip evaluator.")

        # ---- extra validations ----
        if getattr(self, 'extra_evaluators', None):
            for cur_eval_title, cur_evaluator in self.extra_evaluators.items():
                ...
                if len(cur_states) == 0:
                    LOGGER.info(f"Extra val '{cur_eval_title}': no states; skip evaluator.")
                    continue
    ...
    def _do_step(self, batch, batch_idx, mode='train', disc_step=False, extra_val_key=None):
        """Compute losses and metrics. If disc_step=True, run discriminator loss, else generator loss."""
        batch = self(batch)
        total_loss = 0
        metrics = {}

        if not disc_step:  # generator update
            total_loss, metrics = self.generator_loss(batch)
        else:              # discriminator update
            total_loss, metrics = self.discriminator_loss(batch)

        # 可视化逻辑保持不动
        if self.trainer.is_global_zero and (batch_idx % self.visualize_each_iters == 0 or mode == 'test'):
            if self.config.losses.adversarial.weight > 0:
